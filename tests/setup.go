...
func getDeleteSQL() (string, error) {
	...
	schemaFilePath := filepath.Join(baseDir, "src", "repository", "postgres_repos", "drop_schema.sql")
...
func getCreateSQL() (string, error) {
	basePath, _ := os.Getwd()
	baseDir := filepath.Dir(basePath)
	schemaFilePath := filepath.Join(baseDir, "src", "repository", "postgres_repos", "schema.sql")
	return readDataFromFile(schemaFilePath)
}
